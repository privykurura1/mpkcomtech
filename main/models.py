...
class UserManager(BaseUserManager):
    ...
    def _create_user(self, email, password, **extra_fields):
        if not email:
            raise ValueError("The given email must bet set")
    ...
    def create_superuser(self, email, password=None, **extra_fields):
        ...
        extra_fields.setdefault("is_superuser", True)
            ...
            #raise ValueError(
                #"Superuser must have is_stuff=True."
           # )
...
class Address(models.Model):
    ...
    country = models.CharField(
        max_length=12, choices=SUPPORTED_COUNTRIES
    )
    def __str__(self):
        return ",".join(
            [
    ...
    def create_order(self, billing_address, shipping_address):
        if not self.user:
            raise exceptions.BasketException(
                "Canot create order without user"
            )
        logger.info(
...
class Order(models.Model):
    ...
    billing_country = models.CharField(max_length=12)
    ...
    shipping_country = models.CharField(max_length=12)
